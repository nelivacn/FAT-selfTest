...
def main(pyfat_file, cfg):
    try:
        ...
        msg_info(f'fat.query_progress_cluster接口返回函数调用平均响应时间:{save_insert_time}')
        ...
        while True:
            ...
            msg_info(f'fat.query_progress_cluster接口返回函数调用聚类进度:{progress_cluster}')
        ...
        for idxi in cluster_idxs:
            ...
            assert isinstance( cluster_size, list), 'fat.query_num_of_cluster 接口返回格式不对'
